...
public interface MeldungObservationTagRepository extends JpaRepository<MeldungObservationTag, Long> {
    ...
    @Query("""
           ...
           """)
    List<MeldungObservationTag> findAllByObservationIds(@Param("observationIds") List<Long> observationIds);
    ...
    @Query("""
           ...
           where t.observation.id in :observationIds
           """)
    void deleteAllByObservationIds(@Param("observationIds") List<Long> observationIds);
}