...
import {
    ...
    type ObservationTagDraft,
    type JugendamtInformiert,
    type Sichtbarkeit,
    type Meldeweg,
    type Dringlichkeit,
    type Datenbasis,
    type AmpelStatus,
    type JugendamtKontaktart,
    type ObservationQuelle,
    type ObservationOrt,
    type ObservationZeitraum,
...
// ------------------------------------------------------------
// Anlass-Katalog (Frontend) – 1:1 passend zum Backend AnlassCatalog.CODES
// Anzeige ist deutsch (labels), Payload bleibt code.
// ------------------------------------------------------------
...
const DRING: Dringlichkeit[] = ["AKUT_HEUTE", "ZEITNAH_24_48H", "BEOBACHTEN", "UNKLAR"];
const DATENB: Datenbasis[] = ["EIGENE_AUFTRAGSERFUELLUNG", "EINWILLIGUNG_LIEGT_VOR", "GESETZL_GRUNDLAGE", "UNGEKLAERT"];
const AMPEL: AmpelStatus[] = ["GRUEN", "GELB", "ROT", "UNKLAR"];

const JUG_INF: JugendamtInformiert[] = ["JA", "NEIN", "NOCH_NICHT_ENTSCHIEDEN"];
type YouthContactArt = JugendamtKontaktart;
const JUG_KONTAKTART: YouthContactArt[] = ["TELEFON", "SCHRIFTLICH", "PERSOENLICH", "ONLINE"];

const OBS_QUELLE: ObservationQuelle[] = ["EIGENBEOBACHTUNG", "AUSSAGE_KIND", "AUSSAGE_DRITTE", "DOKUMENT", "SONSTIGE"];
const OBS_ORT: ObservationOrt[] = ["SCHULE", "KITA", "ZUHAUSE", "WEG", "ONLINE", "SONSTIGE"];
const OBS_ZEITRAUM: ObservationZeitraum[] = ["HEUTE", "LETZTE_WOCHE", "LETZTER_MONAT", "LAENGER", "UNBEKANNT"];
const SICHT: Sichtbarkeit[] = ["INTERN", "WEITERGABEHINWEIS"];

function pick<T extends string>(value: string, allowed: T[], fallback: T): T {
    return allowed.includes(value as T) ? (value as T) : fallback;
}

function toDraftFromResponse(v: ErstmeldungResponse): ErstmeldungDraftRequest {
    const observations: ObservationDraft[] = v.observations.map((o) => ({
...
export function ErstmeldungEditor(props: {
    value: ErstmeldungResponse;
    disabled?: boolean;
    onSaveDraft: (req: ErstmeldungDraftRequest) => Promise<ErstmeldungResponse | void>;
    onSubmit: (mirrorObservationsToNotizen: boolean, recomputeRisk: boolean) => Promise<void>;
}) {
    const { value, disabled = false, onSaveDraft, onSubmit } = props;

    const [form, setForm] = React.useState<ErstmeldungDraftRequest>(() => toDraftFromResponse(value));
    const [saving, setSaving] = React.useState(false);
    const [saveMsg, setSaveMsg] = React.useState<string | null>(null);
    const [submitMirror, setSubmitMirror] = React.useState(true);
    const [submitRisk, setSubmitRisk] = React.useState(true);
    const [validationErr, setValidationErr] = React.useState<string | null>(null);

    const set = <K extends keyof ErstmeldungDraftRequest>(k: K, v: ErstmeldungDraftRequest[K]) =>
        setForm((s) => ({ ...s, [k]: v }));
    ...
    const updateObsTag = (obsIdx: number, tagIdx: number, patch: Partial<ObservationTagDraft>) => {
        const arr = [...(form.observations || [])];
    ...
    const validateForSubmitUI = (): string | null => {
        if (!form.kurzbeschreibung || form.kurzbeschreibung.trim().length === 0) return "Kurzbeschreibung fehlt.";
        if (!form.fachAmpel) return "Fachliche Ampel fehlt.";
        if (!form.fachText || form.fachText.trim().length === 0) return "Fachliche Einschätzung (Text) fehlt.";
        if (!form.jugendamt?.informiert) return "Jugendamt-Entscheidung fehlt.";
        if (
            form.jugendamt.informiert !== "JA" &&
            (!form.jugendamt.begruendung || form.jugendamt.begruendung.trim().length === 0)
        ) {
        ...
        if (obs.some((o) => !o.text || o.text.trim().length === 0)) return "Es gibt Observations ohne Text.";
        return null;
    };
    ...
    const doSubmit = async () => {
        ...
        if (vErr) return;
        await doSave();
        await onSubmit(submitMirror, submitRisk);
    };
        ...
        <div className="space-y-4">
            {disabled && (
                <Alert>
                    ...
                    <AlertDescription>
                        Diese Version ist abgeschlossen und nicht mehr editierbar. Erstelle eine neue Version oder klone die aktuelle.
                    </AlertDescription>
                </Alert>
            )}
            ...
            <Tabs defaultValue="basis">
                    ...
                    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <select
                                ...
                                value={form.meldeweg ?? "EIGENBEOBACHTUNG"}
                        ...
                        <div className="space-y-2">
                            ...
                            <select
                                className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm"
                                value={form.dringlichkeit ?? "UNKLAR"}
                        ...
                        <div className="space-y-2">
                            ...
                            <select
                                className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm"
                                value={form.datenbasis ?? "UNGEKLAERT"}
                        ...
                        <div className="flex items-center justify-between rounded-xl border border-border p-3">
                            <div>
                                ...
                                <div className="text-xs text-muted-foreground">Nur Metadaten. Keine Inhalte erzwingen.</div>
                            </div>
                            <Switch
                ...
                <TabsContent value="anlass" className="mt-4 space-y-4">
                    <Alert>
                        ...
                        <AlertDescription>
                            Auswahl wird als stabile Codes gespeichert (Backend-Whitelist). Anzeige ist deutsch.
                        </AlertDescription>
                    </Alert>
                ...
                <TabsContent value="obs" className="mt-4 space-y-4">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                                Jede Observation kann <strong>mehrere Indikator-Zuordnungen</strong> erhalten (Tags).
                            </div>
                        </div>
                        <Button onClick={addObservation} disabled={disabled}>
                            + Observation
                        </Button>
                    </div>
                            ...
                            <Card key={idx} className="rounded-2xl">
                                <CardHeader className="flex-row items-start justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <Button variant="outline" size="sm" onClick={() => addObsTag(idx)} disabled={disabled}>
                                            + Tag
                                        </Button>
                                        <Button variant="destructive" size="sm" onClick={() => removeObs(idx)} disabled={disabled}>
                                            Entfernen
                                        </Button>
                                    </div>
                                </CardHeader>
                                                ...
                                                {(o.tags || []).map((t, tIdx) => (
                                                    <div key={tIdx} className="rounded-xl border border-border p-3">
                                                        ...
                                                        <div className="mt-3 flex items-start gap-3">
                                                            ...
                                                            <Button variant="outline" onClick={() => removeObsTag(idx, tIdx)} disabled={disabled}>
                                                                Entfernen
                                                            </Button>
                                                        </div>
                                                    </div>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            ...
                            {form.jugendamt?.informiert && form.jugendamt.informiert !== "JA" ? (
                                <div className="space-y-1">
                                    <Label>Begründung (Pflicht bei Submit wenn nicht JA)</Label>
                                    <Textarea
                                        rows={3}
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            <div className="flex flex-wrap items-center gap-3">
                                <Button onClick={doSave} disabled={disabled || saving}>
                                ...
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <div className="text-xs text-muted-foreground">
                                            Empfohlen, damit Instrument über Notiz-Tags arbeitet.
                                        </div>
                                    </div>
                                    <Switch checked={submitMirror} onCheckedChange={setSubmitMirror} disabled={disabled} />
                                </div>

                                <div className="flex items-center justify-between">
                                    <div>
                                        <div className="font-medium">Risk neu berechnen</div>
                                        <div className="text-xs text-muted-foreground">Nach Abschluss RiskSnapshot aktualisieren.</div>
                                    </div>
                                    <Switch checked={submitRisk} onCheckedChange={setSubmitRisk} disabled={disabled} />
                                </div>

                                <Button onClick={doSubmit} disabled={disabled || saving}>
                                    Abschließen (Submit)
                                </Button>
                            </div>
                        </CardContent>
