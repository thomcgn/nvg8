...
export default function AkteDetailsPage() {
    ...
    const id = useMemo(() => safeNumber((params as any)?.id), [params]); // params typing in next can be awkward; safeNumber handles it

    const [data, setData] = useState<FalleroeffnungResponse | null>(null);
    const [loading, setLoading] = useState(false);
    const [err, setErr] = useState<string | null>(null);

    const autostartErstmeldung = searchParams?.get("autostart") === "erstmeldung";
    ...
    function goErstmeldung() {
        if (!id) return;
        router.push(`/dashboard/falloeffnungen/${id}/erstmeldung`);
    }
    ...
    // Optional: wenn du nach "Akte starten" direkt in Erstmeldung willst:
    useEffect(() => {
        if (id && autostartErstmeldung) {
            goErstmeldung();
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [id, autostartErstmeldung]);
                    ...
                    <Card>
                        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <div className="text-sm font-semibold text-brand-text">
                                        {data?.aktenzeichen ?? (loading ? "Lade…" : id ? `Fall #${id}` : "Fall")}
                                    </div>
                                    <div className="mt-1 text-xs text-brand-text2">{data?.kindName ? `Kind: ${data.kindName}` : " "}</div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                                ...
                                <Button onClick={goErstmeldung} disabled={loading || !id} className="gap-2">
                                    Erstmeldung {data ? "öffnen" : "starten"}
                        ...
                        <CardContent className="space-y-3">
                            ...
                            <div className="text-sm text-brand-text2">Erstellt: {data?.createdAt ?? "—"}</div>

                            <div className="rounded-2xl border border-brand-border bg-white p-4 text-sm text-brand-text2">
                                Nächster Schritt: Erstmeldung (Wizard) oder Notizen & Statuswechsel:
                                <div className="mt-2 flex flex-wrap gap-2">
                                    <code className="rounded bg-brand-bg px-1">GET /falloeffnungen/{`{id}`}</code>
