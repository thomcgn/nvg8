...
import { FileText, RefreshCw, ArrowRight, ChevronDown, ChevronUp } from "lucide-react";
...
function toneForStatus(
    ...
    if (s.includes("hoch") || s.includes("krit") || s.includes("risiko"))
        return "danger";
    if (s.includes("warn") || s.includes("prüf") || s.includes("review"))
        return "warning";
    if (
        s.includes("abgesch") ||
        s.includes("done") ||
        ...
        return "success";
    if (s.includes("offen") || s.includes("neu") || s.includes("draft"))
        return "info";
    return "neutral";
}
...
function Row({ label, value }: { label: string; value: any }) {
    ...
    return (
        <div className="min-w-0">
            ...
            <div className="text-sm text-brand-text whitespace-pre-wrap break-words">
                {v}
            </div>
        </div>
    );
...
function SimpleTable({
                         ...
                         rows,
                         containerClassName,
                         maxHeightClassName, // e.g. "max-h-[60vh]"
                         stickyHeader,
                     }: {
    ...
    const overflow = maxHeightClassName
            ...
            <table className="w-full text-sm border-separate border-spacing-0">
                ...
                {rows.length ? (
                    rows.map((r, idx) => (
                        <tr
                            key={idx}
                            className={idx % 2 === 0 ? "bg-white" : "bg-brand-bg/30"}
                        >
                            {columns.map((c) => (
                                <td
                                    key={c.key}
                                    className={`px-3 py-2 align-top text-brand-text ${c.className ?? ""}`}
                                >
                                    {r[c.key] ?? "—"}
                                </td>
                            ))}
                    ...
                    <tr>
                        <td
                            className="px-3 py-3 text-brand-text2"
    ...
    return (
        <div className="rounded-2xl border border-brand-border/30 bg-white p-3">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                <div className="min-w-0">
                    <div className="text-xs font-semibold text-brand-text2">
                        Dringlichkeit
                    </div>
                ...
                <div className="min-w-0">
                    ...
                    <div className="text-sm text-brand-text">
                        {d.akutGefahrImVerzug ? "Gefahr im Verzug" : "—"}
                    </div>
                </div>
                ...
                <div className="min-w-0">
                    ...
                    <div className="text-sm text-brand-text">
                        {formatDateDE(d.naechsteUeberpruefungAm)}
                    </div>
                </div>
...
function MeldungDetailsFlat({ d }: { d: MeldungResponse }) {
    const [tab, setTab] = useState<
        "inhalt" | "meta" | "fach" | "akut" | "planung" | "listen" | "audit"
    >("inhalt");
                    ...
                    <Section title="Meta">
                        <KeyGrid>
                            ...
                            <Row
                                ...
                                value={d.schweigepflichtentbindungVorhanden}
                            />
                        </KeyGrid>
                    </Section>
                ) : null}
                    ...
                    <Section title="Planung">
                        <KeyGrid>
                            <Row
                                ...
                                value={d.verantwortlicheFachkraftUserId}
                            />
                ...
                {tab === "audit" ? (
                    <div className="space-y-6">
                        ...
                        <Section title="Änderungen">
                            {/* ✅ nur dieser Bereich scrollt; Header sticky */}
                            <SimpleTable
                                stickyHeader
...
export default function AkteDetailsPage() {
    ...
    const fallId = useMemo(() => safeNumber((params as any)?.fallId), [params]);
    ...
    async function loadFall() {
        ...
        try {
            const res = await apiFetch<FalleroeffnungResponse>(`/falloeffnungen/${fallId}`, {
                method: "GET",
            });
            setData(res);
        } catch (e: unknown) {
    ...
    async function prefetchMeldungDetails(list: MeldungListItemResponse[]) {
        ...
        await Promise.allSettled(
            slice.map(async (m) => {
                const d = await meldungApi.get(fallId, m.id);
                setMeldungen((prev) =>
                    prev.map((x) => (x.id === m.id ? { ...x, detail: x.detail ?? d } : x))
                );
            })
        );
    }
    ...
    async function loadMeldungen() {
        ...
        try {
            const res: any = await meldungApi.list(fallId);
            const list: MeldungListItemResponse[] = Array.isArray(res)
                ? res
                : (res?.items ?? res?.data ?? res?.meldungen ?? []);
    ...
    async function ensureDetail(m: MeldungVM) {
        ...
        try {
            const d = await meldungApi.get(fallId, m.id);
            setMeldungen((prev) =>
                prev.map((x) => (x.id === m.id ? { ...x, detail: d } : x))
            );
        } catch (e) {
    ...
    useEffect(() => {
        if (fallId && autostartMeldungen) {
            console.log("[AkteDetailsPage] autostartMeldungen = true (no redirect here)");
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
                            ...
                            <div className="flex items-start gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                    <div className="text-base font-semibold text-brand-text truncate">
                                        {data?.aktenzeichen ??
                                            (loading ? "Lade…" : fallId ? `Fall #${fallId}` : "Fall")}
                                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                            {meldungen.map((m) => {
                                ...
                                const createdBy =
                                    d?.createdByDisplayName ?? m.createdByDisplayName ?? "Unbekannt";
                                        ...
                                        >
                                            <div className="flex items-start gap-3">
                                                    ...
                                                    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                                        ...
                                                        >
                                                            ...
                                                            {m.current ? (
                                                                ...
                                                                <Badge tone="neutral">v{m.versionNo}</Badge>
                                                            )}
                                                            {m.correctsId ? (
                                                                <Badge tone="warning">korrigiert #{m.correctsId}</Badge>
                                                            ) : null}
                                                            {m.supersedesId ? (
                                                                <Badge tone="neutral">ersetzt #{m.supersedesId}</Badge>
                                                            ) : null}

                                                            {!isReadOnlyStatus(m.status) ? (
                                                                <Button
                                                                    onClick={(e) => {
                                                                        stop(e);
                                                                        router.push(
                                                                            `/dashboard/falloeffnungen/${fallId}/meldungen/${m.id}`
                                                                        );
                                                                    }}
                                                ...
                                                <div className="shrink-0 mt-0.5">
                                                    {isOpen ? (
                                                        <ChevronUp className="h-7 w-7 text-brand-text2" />
                                                    ) : (
                                                        <ChevronDown className="h-7 w-7 text-brand-text2" />
                                                    )}
                                                </div>
                                            </div>
                                        </CardHeader>
