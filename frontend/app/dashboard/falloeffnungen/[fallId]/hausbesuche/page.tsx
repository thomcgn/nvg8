        ...
        hausbesuchApi
            ...
            .then((data) => {
                const sorted = [...data].sort(
                    (a, b) =>
                        new Date(b.besuchsdatum).getTime() -
                        new Date(a.besuchsdatum).getTime()
                );
                setItems(sorted);
            })
            .catch(() => setErr("Hausbesuche konnten nicht geladen werden."))
    ...
    return (
        <AuthGate>
            <div className="min-h-screen bg-brand-bg overflow-x-hidden">
                <Topbar title="Hausbesuchsprotokolle" />
                ...
                <div className="mx-auto w-full max-w-4xl px-3 sm:px-6 pb-12 pt-4 space-y-4">
                    ...
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <Button
                            className="gap-2"
                            onClick={() =>
                                router.push(`/dashboard/falloeffnungen/${fid}/hausbesuche/neu`)
                            }
                        >
                    ...
                    <div className="flex items-center gap-3 px-1">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-brand-text2 mt-0.5">
                                {loading
                                    ? "Lade…"
                                    : `${items.length} Protokoll${items.length !== 1 ? "e" : ""}`}
                            </div>
                        </div>
                    </div>
                    ...
                    {!loading && items.length === 0 && !err && (
                        <Card>
                            <CardContent className="py-8 text-center text-sm text-brand-text2">
                                Noch kein Hausbesuch dokumentiert.
                                <div className="mt-3">
                                    <Button
                                        onClick={() =>
                                            router.push(
                                                `/dashboard/falloeffnungen/${fid}/hausbesuche/neu`
                                            )
                                        }
                                        className="gap-2"
                                    >
                    ...
                    <div className="space-y-3">
                        {items.map((item) => (
                            <HausbesuchCard
                                ...
                                onClick={() =>
                                    router.push(
                                        `/dashboard/falloeffnungen/${fid}/hausbesuche/${item.id}`
                                    )
                                }
                            />
                        ))}
...
function HausbesuchCard({
    item,
    onClick,
}: {
    item: HausbesuchListItem;
    onClick: () => void;
}) {
    const ampelDot = item.einschaetzungAmpel
        ? (AMPEL_DOT[item.einschaetzungAmpel] ?? "bg-gray-400")
        : null;
    const ampelLabel = item.einschaetzungAmpel
        ? (AMPEL_LABELS[item.einschaetzungAmpel] ?? item.einschaetzungAmpel)
        : null;
                ...
                <div className="min-w-0 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-sm font-semibold text-brand-text">
                            {new Date(item.besuchsdatum).toLocaleDateString("de-DE")}
                        </span>
                        {ampelDot && ampelLabel && (
                            <span className="flex items-center gap-1.5 text-xs text-brand-text2">
                                <span
                                    className={`inline-block h-2.5 w-2.5 rounded-full ${ampelDot}`}
                                />
                                {ampelLabel}
                            </span>
                        )}
